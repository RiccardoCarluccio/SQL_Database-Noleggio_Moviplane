...
{
    internal class Automobile : Veicoli
    {
        private char tipo;
        public Automobile(string codice, string descrizione, string targa, string marca, int posti, char tipo, double prezzo, bool disponibile) : base(codice, descrizione, targa, marca, posti, prezzo, disponibile)
        {
            Codice = codice;
        ...
        public override void Inserimento(SqlConnection Cn)
        {
            try
            {
                string query = $"INSERT INTO [Anagrafica Automobili] VALUES ('{Codice}', '{Descrizione}', '{Targa}', '{Marca}', {Posti}, {Prezzo}, '{Disponibile}', '{Tipo}')";
        ...
        public override void Cancellazione(SqlConnection Cn)
        {
            try
            {
                string query = $"DELETE FROM [Anagrafica Automobili] WHERE Codice = '{Codice}'";
                SqlCommand cmd = new SqlCommand(query, Cn);
                cmd.ExecuteNonQuery();
            }
            catch (IOException ex)
        ...
        public override void Stampa(SqlConnection Cn)
        {
            try
            {
                string query = "SELECT * FROM [Anagrafica Automobili]";
                SqlCommand cmd = new SqlCommand(query, Cn);
        ...
        public override void StampaVeicolo(SqlConnection Cn)
        {
            try
            {
                string query = $"SELECT * FROM [Anagrafica Automobili] WHERE Codice = '{Codice}'";
                SqlCommand cmd = new SqlCommand(query, Cn);
